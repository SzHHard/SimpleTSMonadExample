...
export const testMonads = () => {
    ...
    describe('Check monadic laws', () => {
        ...
        it('should be assosiative', () => {
            ...
            const first = new Some(3).flatMap(val => (new Some(val + 2).flatMap(f))).get()
            const second = new Some(3).flatMap(val => new Some(val + 2)).flatMap(f).get()
            assert.equal(first, second)
        })
        it('should recognize Some', () => {
            const shouldBeFalse = divide(10, 0).flatMap(res => new Some(String(res)).flatMap(res => new Some(res + 'abc'))).isSome()
            const shouldBeTrue = divide(10, 2).flatMap(res => new Some(String(res)).flatMap(res => new Some(res + 'abc'))).isSome()
